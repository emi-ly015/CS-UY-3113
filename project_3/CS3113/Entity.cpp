...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mDirection {RIGHT}, mFrameSpeed {0} { }
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath) : 
    ...
    mDirection {RIGHT},mFrameSpeed {0}, mSpeed {DEFAULT_SPEED}, mAngle {0.0f} { }

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, std::vector<int> animationAtlas) : mPosition {position}, 
        mVelocity {0.0f, 0.0f}, mAcceleration {0.0f, 0.0f}, 
        ...
        mAnimationIndices {animationAtlas}, mDirection {RIGHT},
    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (mVelocity.y > 0) 
            {
                ...
                mCollidedEntity = collidableEntity;
...
void Entity::update(float deltaTime, Entity *collidableEntities, int collisionCheckCount)
{
    ...
    // when we land, stop movement but only for the player
    if ((mGameStatus == FAILED || mGameStatus == ACCOMPLISHED) && !mIsMovingPlatform)
    {
        mVelocity = { 0.0f, 0.0f };
    ...
    // only accelerate when there is fuel
    if (mFuel > 0.0f)  
    {
        if (hasFuel())
        {
            bool isAccelerating = (mAcceleration.x != 0.0f) || (mAcceleration.y < ACCELERATION_OF_GRAVITY);
            
            if (isAccelerating)
            {
                float fuelUsed = mFuelRate * deltaTime;
                useFuel(fuelUsed);
            ...
            // fuel runs out
            mVelocity.y += ACCELERATION_OF_GRAVITY * deltaTime;
        } 
    } else {
        // entities without fuel
        mVelocity.x += mAcceleration.x * deltaTime;
        mVelocity.y += mAcceleration.y * deltaTime;
    }
    ...
    if (mGameStatus == ONGOING && mIsCollidingBottom && mCollidedEntity != nullptr)
    {
        mGameStatus = mCollidedEntity->getGameStatus();
        // movement stops when landed
        mVelocity = { 0.0f, 0.0f };
        mAcceleration = { 0.0f, 0.0f };
    }
...
void Entity::makeMovingPlatform(float speed, float distance)
{
    mIsMovingPlatform = true;
    mMoveSpeed = speed;
    mMoveDistance = distance;
    mStartPoint = mPosition.x;
}

// update movement for the moving platform
void Entity::updatePlatform(float deltaTime)
{
    // movement only happens for moving platform
    if (!mIsMovingPlatform) return;
    if (mMovingRight) mPosition.x += (mMoveSpeed * deltaTime);
    else mPosition.x -= (mMoveSpeed * deltaTime);

    // move side to side
    if (mPosition.x > mStartPoint + mMoveDistance) mMovingRight = false;
    if (mPosition.x < mStartPoint - mMoveDistance) mMovingRight = true;
}