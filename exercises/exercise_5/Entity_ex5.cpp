    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (mVelocity.y > 0) 
            {
                ...
                // deactivate block when hit
                if (collidableEntity->mEntityType == BLOCK && mEntityType == PLAYER){
            ...
            if (mEntityType == PLAYER && collidableEntity->mEntityType == BLOCK && collidableEntity->isActive())
            {
                collidableEntity->deactivate();
            }
        }
...
void Entity::update(float deltaTime, Entity *collidableEntities, 
    int collisionCheckCount, Entity *block, int blockCountHit)
{
    ...
    if (block != nullptr && blockCountHit > 0) checkCollisionY(block, blockCountHit);
    if (block != nullptr && blockCountHit > 0) checkCollisionX(block, blockCountHit);
