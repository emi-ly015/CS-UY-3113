...
void render();
void shutdown();
bool blocksDeactivated();


bool blocksDeactivated(){
    for (int i = 0; i < NUMBER_OF_BLOCKS; i ++)
    {
        if ( gBlocks[i].isActive()) return false;
    }
    return true;
}
...
void update() 
{
    ...
    bool allDeactivated = blocksDeactivated();
    for (int i = 0; i < NUMBER_OF_TILES; i++)
        {
            if (allDeactivated && gTiles[i].getEntityType() == PLATFORM) gTiles[i].deactivate();
            gTiles[i].update(FIXED_TIMESTEP, nullptr, 0 ,nullptr, 0);
        }
