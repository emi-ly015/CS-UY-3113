...
void update() 
{
    ...
    if (gTeardropStatus == HANGING) {
        ...
        // Add vertical translation for teardrop
        gPosition.y += SPEED * deltaTime;

        // If teardrop is colliding with beaker, shrink the size of teardrop frame-independently
        bool collision = isColliding(
            &gPosition, &gScale, 
