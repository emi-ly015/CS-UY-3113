...
// added flyer to meet 3 movement requirements
...
public:
    ...
    bool enemyCollision(Entity *other) { return isColliding(other); }
    ...
    void flyDirection(float left, float right)
        {
            if (mPosition.x < left) setDirection(RIGHT);
            else if (mPosition.x > right) setDirection(LEFT);
        }
