...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mSpriteSheetDimensions {}, mAnimation {WALK_DOWN}, 
                   mPreviousDirection {WALK_DOWN}, mAnimationAtlas {{}}, 
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
    ...
    mTextureType {SINGLE}, mAnimation {WALK_DOWN}, mPreviousDirection {WALK_DOWN}, 
...
Entity::Entity(Vector2 position, Vector2 scale, std::map<Animation, std::string> textures,
        ...
        mAnimationAtlas {animationAtlas}, mAnimation {WALK_DOWN}, 
        mPreviousDirection {WALK_DOWN}, 
        mAnimationIndices {animationAtlas.at(WALK_DOWN)}, 
        ...
        mSpeed { DEFAULT_SPEED }, mEntityType {entityType} 
{
    ...
    mTextures[WALK_UP]  = LoadTexture(textures[WALK_UP].c_str());
    mTextures[WALK_DOWN]  = LoadTexture(textures[WALK_DOWN].c_str());

    mTextures[PLAYER_IDLE]  = LoadTexture(textures[PLAYER_IDLE].c_str());
    mTextures[PLAYER_SHOVEL_FRONT] = LoadTexture(textures[PLAYER_SHOVEL_FRONT].c_str());
    mTextures[PLAYER_SHOVEL_BACK]   = LoadTexture(textures[PLAYER_SHOVEL_BACK].c_str());

    mTextures[PLAYER_AXE_FRONT]   = LoadTexture(textures[PLAYER_AXE_FRONT].c_str());
    mTextures[PLAYER_AXE_BACK]   = LoadTexture(textures[PLAYER_AXE_BACK].c_str());
    mTextures[PLAYER_AXE_RIGHT]   = LoadTexture(textures[PLAYER_AXE_RIGHT].c_str());
    mTextures[PLAYER_AXE_LEFT]   = LoadTexture(textures[PLAYER_AXE_LEFT].c_str());
    mTextures[PLAYER_WATER_FRONT]   = LoadTexture(textures[PLAYER_WATER_FRONT].c_str());
    mTextures[PLAYER_WATER_BACK]   = LoadTexture(textures[PLAYER_WATER_BACK].c_str());
    mTextures[PLAYER_WATER_RIGHT]   = LoadTexture(textures[PLAYER_WATER_RIGHT].c_str());
    mTextures[PLAYER_WATER_LEFT]   = LoadTexture(textures[PLAYER_WATER_LEFT].c_str());
...
{ 
    ...
    UnloadTexture(mTextures[PLAYER_IDLE]);
    UnloadTexture(mTextures[PLAYER_SHOVEL_FRONT]);
    UnloadTexture(mTextures[PLAYER_SHOVEL_BACK]);
    UnloadTexture(mTextures[PLAYER_AXE_FRONT]);
    UnloadTexture(mTextures[PLAYER_AXE_BACK]);
    UnloadTexture(mTextures[PLAYER_AXE_RIGHT]);
    UnloadTexture(mTextures[PLAYER_AXE_LEFT]);
    UnloadTexture(mTextures[PLAYER_WATER_FRONT]);
    UnloadTexture(mTextures[PLAYER_WATER_BACK]);
    UnloadTexture(mTextures[PLAYER_WATER_RIGHT]);
    UnloadTexture(mTextures[PLAYER_WATER_LEFT]);
    UnloadTexture(mTextures[WALK_RIGHT]);
    UnloadTexture(mTextures[WALK_UP]);
    UnloadTexture(mTextures[WALK_DOWN]);
...
void Entity::animate(float deltaTime)
{
    ...
    bool isToolAnimation =
    mAnimation == PLAYER_SHOVEL_FRONT || mAnimation == PLAYER_SHOVEL_BACK ||
    mAnimation == PLAYER_AXE_FRONT    || mAnimation == PLAYER_AXE_BACK    ||
    mAnimation == PLAYER_AXE_LEFT     || mAnimation == PLAYER_AXE_RIGHT   ||
    mAnimation == PLAYER_WATER_FRONT  || mAnimation == PLAYER_WATER_BACK  ||
    mAnimation == PLAYER_WATER_LEFT   || mAnimation == PLAYER_WATER_RIGHT;

    if (isToolAnimation)
    {
        // still animate frames but do not replace animation with idle or walk
        mAnimationIndices = mAnimationAtlas.at(mAnimation);
        mCurrentTexture = mTextures.at(mAnimation);
        mCurrentSpriteSheetDimensions = mSpriteSheetDimensions.at(mAnimation);
    ...
    if (GetLength(mMovement) > 0)
    {
        if (fabs(mMovement.x) > fabs(mMovement.y))
            mAnimation = (mMovement.x < 0) ? WALK_LEFT : WALK_RIGHT;
        else mAnimation = (mMovement.y < 0) ? WALK_UP : WALK_DOWN;
    } else  mAnimation = PLAYER_IDLE;
...
Animation Entity::getFacingAnimation()
{
    // if moving, determine direction 
    if (GetLength(mMovement) > 0)
    {
        if (fabs(mMovement.x) > fabs(mMovement.y))
        {
            if (mMovement.x < 0) mPreviousDirection = WALK_LEFT;
            else mPreviousDirection = WALK_RIGHT;
        }
        else
        {
            if (mMovement.y < 0)  mPreviousDirection = WALK_UP;
            else  mPreviousDirection = WALK_DOWN;
        }
    }
