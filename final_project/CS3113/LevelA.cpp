...
void LevelA::initialise()
{
   ...
   mission_wheat = false;
   mission_turnip = false;
   mission_peaches = false;
   mission_pet = false;
   ...
   starTexture = LoadTexture("assets/project5_assets/level1_stars.png");
   ...
   */
   ...
   // starts as wanderer but becomes a follower when a pet
...
void LevelA::update(float deltaTime)
{
   ...
   // win condition
   if (mission_pet && mission_wheat && mission_turnip
       && mission_peaches) mGameState.nextSceneID = 2;
...
void LevelA::drawUI()
{
   ...
   */
   ...
   DrawTexturePro(playerFace, faceSrc,
      faceDst, {0,0}, 0, WHITE );
   ...
   */
   if (mMissionOpen)
   {
      ...
      DrawTexturePro(
         mMissionBox, boxSource, boxDest, { 0,0 },
         0, WHITE );
      // -------- Missions Text --------
      float textX = boxDest.x + 55;
      float textY = boxDest.y + 50;
      if (mission_pet) DrawTexture(missionDoneTex, boxDest.x + 30, textY, BROWN);
      
      DrawText("Obtain a pet", textX, textY, 18, BROWN);
      textY += 45;
      ...
      if (mission_turnip) DrawTexture(missionDoneTex, boxDest.x + 30, textY, BROWN);
      DrawText("Plant & harvest", textX, textY, 18, BROWN);
      textY += 22;
      DrawText("3 turnip", textX, textY, 18, BROWN);
      textY += 45;

      if (mission_peaches) DrawTexture(missionDoneTex, boxDest.x + 30, textY, BROWN);
      DrawText("Collect 3 peaches", textX, textY, 18, BROWN);
   }
...
void LevelA::shutdown()
{
   ...
   delete mGameState.secondButterfly;
   delete mGameState.cow;
   delete mGameState.chicken;
   delete mGameState.clerk;
}

bool LevelA::isDirtTile(Vector2 worldPos)
{
...
bool LevelA::isWaterTile(EntityDirection* entity, Vector2 worldPos, Direction moveDirection)
{
...
Vector2 LevelA::getDirtCenter(Vector2 worldPos)
{
...
bool LevelA::plantExistsAt(Vector2 pos)
{
...
void LevelA::updatePlantGrowth(float deltaTime)
{
   for (int i = 0; i < mPlants.size(); i++)
   {
      if (mPlants[i].harvested) continue;
   ...
   if (IsKeyPressed(KEY_H))
   {
      ...
      for (int i =0; i < mPlants.size(); i++)
      {
            ...
            fabs(mPlants[i].position.y - tilePos.y) < 1.0f)
         {
            ...
            if (turnipCrops >= 3) mission_turnip = true;
...
void LevelA::updateTreesAnimation(float deltaTime)
{
...
void LevelA::updateTreeInteraction()
{
...
void LevelA::updateTreeCollection()
{
   Vector2 playerPosition = mGameState.player->getPosition();
...
void LevelA::drawInventoryItems()
{
...
void LevelA::addTree(int col, int row, int type)
{
   float tileSize = mGameState.map->getTileSize();
   float left = mGameState.map->getLeftBoundary();
   float top = mGameState.map->getTopBoundary();
...
void LevelA::addToInventory(int itemType, int amount, int &counter)
{
   int existingSlot = -1;
...
void LevelA::treeCollision(EntityDirection* entity, Vector2& lastSafePos)
{
...
void LevelA::playerTreeCollision()
{
...
void LevelA::removeFromInventory(int itemType, int amount, int &counter)
{
   for (int slot = 0; slot < 8 && amount > 0; slot++)
...
void LevelA::chickenFeeding()
{
   // feeding: turn chicken into pet follower
   if (IsKeyPressed(KEY_F) && !chickenIsPet)
   {
      Vector2 playerPos  = mGameState.player->getPosition();
      Vector2 chickenPos = mGameState.chicken->getPosition();
      float distance     = Vector2Distance(playerPos, chickenPos);

      if (distance < 80.0f && wheatCrops > 0)
      {
         wheatCrops--;
         chickenIsPet = true;
         mission_pet = true;
         ...
         // play pet animation briefly 
         mGameState.chicken->setDirection(PET);
      }
   }
}

void LevelA::cowBehavior(float deltaTime)
{
   cowLastSafePos = mGameState.cow->getPosition();
   if (!cowEating)
   {
      cowDecisionTimer +=deltaTime;
      if (cowDecisionTimer >= 4.0f)
      {
         cowDecisionTimer = 0.0f;

         // referenced GetRandomValue from https://www.raylib.com/cheatsheet/cheatsheet.html
         int choice = GetRandomValue(0,9);
         if      (choice <= 2) mGameState.cow->setDirection(LEFT_DIRECTION);
         else if (choice <= 5) mGameState.cow->setDirection(RIGHT_DIRECTION);
         else if (choice <= 7) mGameState.cow->setDirection(UP_DIRECTION);
         else  mGameState.cow->setDirection(DOWN_DIRECTION);

      }
   } else 
   {
      // when eating, keep the cow still
      cowEatingTimer += deltaTime;
      if (cowEatingTimer >= 0.6f)
      {
         cowEating = false;
         cowEatingTimer = 0.0f;

         // when done eating, go back to a walk direction
         Direction facing = mGameState.cow->getDirection();
         if (facing == EAT_LEFT_DIRECTION)
            mGameState.cow->setDirection(LEFT_DIRECTION);
         else if (facing == EAT_RIGHT_DIRECTION)
            mGameState.cow->setDirection(RIGHT_DIRECTION);
      }
   }
   Vector2 cowPos = mGameState.cow->getPosition();
   Vector2 cowFeetPos = {cowPos.x, cowPos.y + mGameState.cow->getColliderDimensions().y / 2.0f };

   if (isWaterTile(mGameState.cow, cowFeetPos, mGameState.cow->getDirection()))
   {
      mGameState.cow->setPosition(cowLastSafePos);
      Direction currentDir = mGameState.cow->getDirection();
      switch (currentDir)
      {
      case LEFT_DIRECTION:
         mGameState.cow->setDirection(RIGHT_DIRECTION);
      ...
      case RIGHT_DIRECTION:
         ...
         break;
      case UP_DIRECTION:
         mGameState.cow->setDirection(DOWN_DIRECTION);
         break;
      case DOWN_DIRECTION:
         mGameState.cow->setDirection(UP_DIRECTION);
         break;
      default:
         break;
      }
   ...
   treeCollision(mGameState.cow, cowLastSafePos);

   // only eats a crop if it exists
   if (!cowEating && isDirtTile(cowPos))
   {
      Vector2 tileCenter = getDirtCenter(cowPos);
      ...
      for (int i = 0; i < mPlants.size(); i++)
      {
         ...
         if (fabs(mPlants[i].position.x - tileCenter.x) < 1.0f &&
             fabs(mPlants[i].position.y - tileCenter.y) < 1.0f)
            {
               ...
               mGameState.cow->setDirection(eatDir);
               cowEating      = true;
