...
bool Map::isSolidTileAt(Vector2 position, float *xOverlap, float *yOverlap)
{
    ...
    *yOverlap = 0.0f;
    ...
    bool isSolid = (tile == 0) || (tile >= 43 && tile <= 49) || (tile == 51);
...
int Map::getTileAtPosition(Vector2 pos)
{
    if (pos.x < mLeftBoundary || pos.x > mRightBoundary ||
        pos.y < mTopBoundary  || pos.y > mBottomBoundary)
        return 0;
    
    int tileX = floor((pos.x - mLeftBoundary) / mTileSize);
    int tileY = floor((pos.y - mTopBoundary) / mTileSize);
