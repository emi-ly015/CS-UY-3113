...
void Effects::update(float deltaTime, Vector2 *viewOffset)
{
    ...
    case FADEOUT:
        ...
        {
            mAlpha = SOLID;
            mCurrentEffect = NONE; // optional
        }
        break;

    case GROW:
        mOverlayHeight += mEffectSpeed * deltaTime *  SIZE_SPEED_MULTIPLIER * diagonalRatio;
        mOverlayWidth += mEffectSpeed * deltaTime *  SIZE_SPEED_MULTIPLIER;
    ...
    case SHRINK:
        ...
        if (mOverlayHeight <= 0.0f || mOverlayWidth <= 0.0f)
        {
            mOverlayHeight = 0.0f;
            mOverlayWidth = 0.0f;
            mCurrentEffect = NONE;
